...
class TestBaseAgent:
    ...
    async def test_handle_task_request_success(self, test_agent):
        ...
        test_message = AgentMessage(
            ...
            payload={"task_type": "test_task", "parameters": {"param1": "value1"}},
        )
...
class TestSelfHealingAgent:
    ...
    async def test_check_service_health_task(self, healing_agent):
        ...
        healing_agent._check_docker_services = AsyncMock(
            return_value={
        ...
        result = await healing_agent._execute_task(
            "check_service_health", {"service": "all"}
        )
    ...
    async def test_restart_service_task(self, mock_subprocess, healing_agent):
        ...
        healing_agent._check_single_service = AsyncMock(
            return_value={"status": "running", "healthy": True}
        )

        result = await healing_agent._execute_task(
            "restart_service", {"service": "fastapi_app"}
        )
    ...
    async def test_cleanup_disk_space_task(self, mock_subprocess, healing_agent):
        ...
        healing_agent._get_disk_usage = AsyncMock(
            return_value=70.0
        )  # 70% usage after cleanup
    ...
    async def test_optimize_memory_task(self, mock_subprocess, healing_agent):
        ...
        # Mock helper methods
        healing_agent._get_container_memory_usage = AsyncMock(
            return_value={
    ...
    async def test_handle_service_failure(self, healing_agent):
        ...
        await healing_agent._handle_service_failure(
            "fastapi_app", {"status": "stopped", "healthy": False}
        )
...
class TestAgentIntegration:
    ...
    async def test_agent_task_execution_flow(self, mock_broker):
        ...
        agent._check_docker_services = AsyncMock(
            return_value={"fastapi_app": {"status": "running", "healthy": True}}
        )
