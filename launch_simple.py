...
def check_prerequisites():
    ...
    try:
        ...
        logger.error(f"{Color.RED}Docker or Docker Compose not available: {e}{Color.NC}")
        return False
...
def start_core_services():
    ...
    try:
        ...
        logger.warning(f"{Color.YELLOW}PostgreSQL might already be running: {e}{Color.NC}")
    ...
    for service_name, (host, port) in services.items():
        if port:
            ...
            for attempt in range(30):
                ...
                logger.warning(f"{Color.YELLOW}{service_name} might not be ready{Color.NC}")
        else:
            # HTTP check
            for attempt in range(30):
                try:
...
def start_fastapi():
    ...
    try:
        ...
        ], cwd=script_dir, env=env, check=False, capture_output=True)
        ...
        process = subprocess.Popen([
            ...
            "--port", "8058",
            "--reload"
        ], cwd=script_dir / "src" / "fastapi_app", env=env)
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(f"{Color.YELLOW}FastAPI responded with status {response.status_code}{Color.NC}")
                return process
        except Exception as e:
            logger.warning(f"{Color.YELLOW}Could not check FastAPI health: {e}{Color.NC}")
            return process
    ...
    for service, url in services.items():
        try:
            if service == "FastAPI":
                ...
                status = f"{Color.GREEN}HEALTHY{Color.NC}" if response.status_code == 200 else f"{Color.YELLOW}RESPONDING{Color.NC}"
            else:
                ...
                sock.close()
                status = f"{Color.GREEN}RUNNING{Color.NC}" if result == 0 else f"{Color.RED}DOWN{Color.NC}"
        except Exception as e:
        ...
        try:
            while True:
                ...
                if fastapi_process and fastapi_process.poll() is not None:
                    logger.warning(f"{Color.YELLOW}FastAPI process has stopped{Color.NC}")
                    break
