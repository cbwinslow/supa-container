...
class TestServiceHealth:
    ...
    def test_supabase_health(self, service_endpoints):
        ...
        try:
            ...
            ), f"Supabase health check failed: {response.status_code}"
    ...
    def test_n8n_health(self, service_endpoints):
        ...
        try:
            ...
            assert (
                response.status_code < 500
            ), f"n8n health check failed: {response.status_code}"
        except requests.exceptions.RequestException as e:
    ...
    def test_flowise_health(self, service_endpoints):
        ...
        try:
            ...
            assert (
                response.status_code < 500
            ), f"Flowise health check failed: {response.status_code}"
        except requests.exceptions.RequestException as e:
    ...
    def test_neo4j_health(self, service_endpoints):
        ...
        try:
            ...
            assert (
                response.status_code < 500
            ), f"Neo4j health check failed: {response.status_code}"
        except requests.exceptions.RequestException as e:
...
class TestAPIEndpoints:
    ...
    def test_fastapi_health_endpoint(self):
        ...
        try:
            ...
            ), f"FastAPI health check failed: {response.status_code}"
    ...
    def test_nextjs_health_endpoint(self):
        ...
        try:
            ...
            assert (
                response.status_code == 200
            ), f"Next.js health check failed: {response.status_code}"
    ...
    def test_supabase_auth_endpoint(self):
        ...
        try:
            response = requests.get(
                "http://localhost:8005/auth/v1/settings", timeout=10
            )
            assert (
                response.status_code == 200
            ), f"Supabase auth check failed: {response.status_code}"
    ...
    def test_memory_usage(self):
        ...
        try:
            ...
            for container in containers:
                ...
                assert (
                    memory_mb < 2048
                ), f"Container {container.name} using too much memory: {memory_mb:.1f}MB"

            # Total usage should be reasonable (less than 8GB for all services)
            assert (
                total_memory_mb < 8192
            ), f"Total memory usage too high: {total_memory_mb:.1f}MB"
            ...
            for container in containers:
                ...
                if "cpu_usage" in cpu_stats and "cpu_usage" in precpu_stats:
                    ...
                    if system_delta > 0:
                        ...
                        assert (
                            cpu_percent < 80
                        ), f"Container {container.name} using too much CPU: {cpu_percent:.1f}%"
        ...
        for service_name, host, port in restricted_services:
            try:
                ...
                if result != 0:
                    ...
                    pytest.skip(
                        f"{service_name} is exposed on port {port} (might be intentional for dev)"
                    )
        ...
        try:
            ...
            for container in containers:
                ...
                if exec_result.exit_code == 0:
                    ...
                    if user_id == "0":
                        pytest.skip(
                            f"Container {container.name} running as root (might be necessary)"
                        )
                    else:
                        assert (
                            True
                        ), f"Container {container.name} running as non-root user {user_id}"
...
class TestIntegrationWorkflows:
    ...
    def test_ai_service_integration(self):
        ...
        try:
            ...
            response = requests.get(
                "http://localhost:8003/api/v1/chatflows", timeout=10
            )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                projects_response = requests.get(
                    "http://localhost:8007/api/public/projects", timeout=10
                )
                # The endpoint might require auth, so we just check it's reachable
                assert (
                    response.status_code < 500
                ), "Langfuse monitoring stack accessible"
            else:
