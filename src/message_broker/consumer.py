...
class MessageConsumer:
    ...
    def __init__(self, broker: RabbitMQBroker, agent_id: str, agent_type: AgentType):
        self.broker = broker
    ...
    async def start_consuming(self):
        ...
        await self.broker.setup_consumer(
            self.agent_id, self.agent_type, self._process_message
        )
    ...
    async def _process_message(self, message: AgentMessage):
        ...
        try:
            logger.debug(
                f"Processing message {message.id} of type {message.message_type}"
            )
    ...
    async def _handle_task_request(self, message: AgentMessage):
        ...
        logger.warning(
            f"Task request received but no handler implemented in {self.agent_type.value} agent"
        )
    ...
    async def send_registration(self):
        ...
        await self.publisher.broadcast_message(
            MessageType.AGENT_REGISTRATION, registration_message
        )
    ...
    def _get_capabilities(self) -> Dict:
        ...
        return {"message_handling": list(self.handlers.keys()), "health_check": True}
