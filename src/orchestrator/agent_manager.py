...
class AgentManager:
    ...
    def update_agent_metrics(self, agent_id: str, metrics: Dict):
        ...
        if len(self.agent_health_history[agent_id]) > 100:
            self.agent_health_history[agent_id] = self.agent_health_history[agent_id][
                -100:
            ]
    ...
    def get_available_agents(self, agent_type: AgentType = None) -> List[str]:
        ...
        for agent_id, agent_data in self.registered_agents.items():
            ...
            if (
                agent_data.get("status") == "running"
                and agent_data.get("health_score", 0) > 50
            ):
    ...
    def get_best_agent_for_task(
        self, task_type: str, agent_type: AgentType = None
    ) -> Optional[str]:
    ...
    def get_system_health_summary(self) -> Dict[str, Any]:
        ...
        if total_agents > 0:
            avg_health = (
                sum(a.get("health_score", 0) for a in self.registered_agents.values())
                / total_agents
            )
