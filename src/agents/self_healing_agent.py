...
class SelfHealingAgent(BaseAgent):
    ...
    async def _monitor_resources(self):
        ...
        while self.running:
            try:
                ...
                if (
                    memory_usage
                    > self.healing_rules["memory_optimization"]["threshold"]
                ):
    ...
    async def _proactive_maintenance(self):
        ...
        while self.running:
            try:
                ...
                if (
                    datetime.utcnow().weekday() == 6 and datetime.utcnow().hour == 3
                ):  # Sunday 3 AM
    ...
    async def _optimize_memory(self, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            high_memory_containers = [
                container
                for container, usage in container_memory.items()
                if usage > 500  # > 500MB
            ]
            ...
            subprocess.run(
                ["echo", "3", ">", "/proc/sys/vm/drop_caches"], shell=True, check=False
            )
    ...
    async def _update_configurations(
        ...
        try:
            ...
            services_to_restart = parameters.get(
                "services", ["fastapi_app", "nextjs_app"]
            )
                ...
                if target == "database":
                    ...
                    subprocess.run(
                        [
                            "tar",
                            "-czf",
                            f"/tmp/config_backup_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.tar.gz",
                            "/opt/supabase-super-stack/.env",
                            "/opt/supabase-super-stack/docker-compose.yml",
                        ],
                        check=False,
                    )
    ...
    async def _clean_directory(self, directory: str) -> float:
        ...
        try:
            # Get initial size
            result = subprocess.run(
                ["du", "-sm", directory], capture_output=True, text=True
            )
            ...
            subprocess.run(
                ["find", directory, "-type", "d", "-empty", "-delete"], check=False
            )
    ...
    async def _clean_old_logs(self):
        ...
        try:
            # Clean Docker logs
            subprocess.run(
                ["docker", "system", "prune", "-f", "--volumes"], check=False
            )
