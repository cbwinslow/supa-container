...
def mock_db_calls():
    with patch(
        "fastapi_app.tools.vector_search", new_callable=AsyncMock
    ) as mock_vector, patch(
        "fastapi_app.tools.hybrid_search", new_callable=AsyncMock
    ) as mock_hybrid:
...
def mock_graph_calls():
    with patch(
        "fastapi_app.tools.search_knowledge_graph", new_callable=AsyncMock
    ) as mock_search:
...
def mock_embedding():
    with patch(
        "fastapi_app.tools.generate_embedding", new_callable=AsyncMock
    ) as mock_embed:
