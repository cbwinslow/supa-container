...
class ServiceManager:
    ...
    def check_prerequisites(self) -> bool:
        ...
        try:
            ...
            if result.returncode != 0:
                logger.error(
                    f"{Color.RED}Docker is not installed or not running{Color.NC}"
                )
                return False
    ...
    def setup_environment(self) -> bool:
        ...
        if env_file.exists():
            ...
            logger.error(
                f"{Color.RED}.env file not found and .env.example not available{Color.NC}"
            )
            return False
    ...
    def _setup_supabase(self) -> bool:
        ...
        if not supabase_dir.exists():
            ...
            try:
                ...
                logger.error(
                    f"{Color.RED}Failed to clone Supabase repository: {e}{Color.NC}"
                )
                os.chdir(old_cwd)
                return False
        else:
    ...
    def start_services(self) -> bool:
        ...
        try:
            ...
            # Start the services
            logger.info(
                f"{Color.BLUE}Starting services with profile: {self.profile}, environment: {self.environment}{Color.NC}"
            )
    ...
    def wait_for_services(self, timeout: int = 300) -> bool:
        ...
        logger.info(
            f"{Color.BLUE}Waiting for services to be ready (timeout: {timeout}s)...{Color.NC}"
        )
        ...
        logger.warning(
            f"{Color.YELLOW}Some services are not ready: {missing_services}{Color.NC}"
        )
        return (
            len(services_ready) > len(self.health_endpoints) // 2
    ...
    def run_comprehensive_tests(self) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                logger.warning(
                    f"{Color.YELLOW}Some Python tests failed, but continuing...{Color.NC}"
                )
    ...
    def _run_service_integration_tests(self) -> bool:
        ...
        logger.info(
            f"{Color.BLUE}Integration test results: {tests_passed}/{tests_total} passed ({success_rate:.1%}){Color.NC}"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Launch Supa Container services locally"
    )
    parser.add_argument(
        "--profile",
        choices=["cpu", "gpu-nvidia", "gpu-amd"],
        default="cpu",
    ...
    parser.add_argument(
        "--no-tests", action="store_true", help="Skip running tests after startup"
    )
    parser.add_argument(
        "--logs",
        type=str,
    ...
    try:
        ...
        if not manager.wait_for_services():
            logger.warning(
                f"{Color.YELLOW}Not all services are ready, but continuing...{Color.NC}"
            )
        ...
        while True:
            time.sleep(30)
            if not manager._check_service_health(
                "supabase", "http://localhost:8005/rest/v1/"
            ):
                logger.warning(
                    f"{Color.YELLOW}Some services may have stopped{Color.NC}"
                )
                manager.show_service_status()
